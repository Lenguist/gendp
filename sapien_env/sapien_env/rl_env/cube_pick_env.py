# File: sapien_env/sapien_env/rl_env/cube_pick_env.py
...
class CubePickRLEnv(CubePickEnv, BaseRLEnv):
    """
    RL wrapper: table + cube + robot, mirroring the HangMug structure,
    but simplified to use only an XArm (no panda fingers).
    """
    def __init__(
        self,
        use_gui=False,
        ...
        **renderer_kwargs
        ...
        if self.is_robot_free:
            ...
            raise NotImplementedError(f"Unknown robot type for {robot_name}")

        self.palm_link_name = info.palm_name
        # find the palm link itself
        self.palm_link = next(
            link for link in self.robot.get_links()
            if link.get_name() == self.palm_link_name
        )

        # XArm has no separate finger links in this setup
        self.finger_tip_links = []

        # Record initial cube pose for oracle
        self.object_episode_init_pose = sapien.Pose()

    def get_oracle_state(self):
        """
        Expert state: robot qpos, cube pose+quat, cube vel+ang, cube_in_palm, vertical cosine
        """
    ...
    def reset(
        ...
        *,
        ...
        if self.is_xarm:
            ...
            # fallback handling (unlikely for this task)
            init_pose = sapien.Pose(
                np.array([-0.4, 0, 0.2]),
        ...
        # Return (observation, oracle_state)
        return self.get_observation(), self.get_oracle_state()

    @cached_property
    def obs_dim(self):
        if not self.use_visual_obs:
            # robot_dof + (pose7 + vel3 + ang3 + in_palm3 + theta1)
